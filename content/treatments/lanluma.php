<?php

/**
 * Content for: Lanluma
 * To edit this page, just change the values in the content_blocks array below!
 * Each block has a "type" that determines which template to use.
 */

return [
    "page_title" => "Lanluma behandeling",

    // Content blocks array - add, remove, or reorder as needed
    "content_blocks" => [
        [
            "type" => "category-intro",
            "data" => [
                "title" => "Wat is Lanluma?",
                "content" => "
                    <p>Lanluma wordt bij onze kliniek in Amsterdam steeds vaker toegepast vanwege de geleidelijke opbouw, waardoor het volume door eigen collageenproductie wordt opgebouwd. Heel geleidelijk en natuurlijk en daarmee een echte biostimulator.</p>
                    <p>Lanluma is een injecteerbare filler en biostimulator op basis van poly-L-melkzuur (PLLA). Dit is een medisch ingrediënt dat al meer dan 30 jaar veilig wordt gebruikt. In tegenstelling tot traditionele fillers zoals hyaluronzuur, werkt Lanluma niet direct \"vullend\", maar stimuleert het je huid om zelf nieuw collageen aan te maken. Het effect ontstaat dus geleidelijk en zorgt voor een natuurlijk ogende versteviging en volumetoename van de huid.</p>
                ",
                "image_filename" => "lanluma-intro.jpg",
                "button_below_text" => true
            ]
        ],
        [
            "type" => "text-content",
            "data" => [
                "title" => "",
                "content" => "
                    <p>Lanluma verschilt wezenlijk van een filler zoals Radiesse. Lanluma is bedoelt voor wie geleidelijk een natuurlijk resultaat wil, terwijl Radiesse juist uitkomst biedt als je direct zichtbaar volume wilt creëren. Het werkende ingrediënt van Radiesse is calciumhydroxylapatiet, dit zorgt voor zowel direct volume alsook voor stimulatie van de eigen collageenproductie. Ook verschilt Lanluma van een biostimulator als Prophilo. Prophilo heeft als werkend ingrediënt hyaluronzuur, hualuronzuur stimuliert met name de huidhydratatie – trekt vocht aan en houdt dit vast – en produceert (in mindere mate) ook collageen- en elastine. Lamluma met als werkend ingrediënt poly-L-melkzuur richt zich puur en alleen op collageenproductie.</p>
                ",
                "show_background" => true
            ]
        ],
        [
            "type" => "navigation",
            "data" => [
                "links" => [
                    ["href" => "#behandeling", "label" => "Behandeling"],
                    ["href" => "#prijzen", "label" => "Prijzen"],
                    ["href" => "#resultaat", "label" => "Resultaat"],
                    ["href" => "#nazorg", "label" => "Nazorg"]
                ]
            ]
        ],
        [
            "type" => "text-content",
            "data" => [
                "title" => "Lanluma voor het Gezicht",
                "content" => "
                    <p>In het gezicht helpt Lanluma bij het herstellen van verloren volume, het verbeteren van de huidstructuur en het verminderen van fijne lijntjes. Het resultaat is een stevigere, gladdere en jeugdigere huid die van binnenuit verjongt, zonder de kunstmatige uitstraling van fillers. Lanluma is ideaal voor het verjongen van de wangen, kaaklijn en het gebied rond de ogen.</p>
                ",
                "show_background" => false,
                "custom_class" => "!py-0",
            ]
        ],
        [
            "type" => "text-content",
            "data" => [
                "title" => "Lanluma voor de Billen",
                "content" => "
                    <p>Lanluma is ook perfect voor het herstellen van volume en stevigheid in de billen. Naarmate we ouder worden of door factoren zoals gewichtsverlies, kan de huid van de billen verslappen en volume verliezen. Lanluma stimuleert de collageenproductie in dit gebied, wat leidt tot een strakkere, vollere en stevigere billen.</p>
                ",
                "show_background" => false
            ]
        ],
        [
            "type" => "text-content",
            "data" => [
                "title" => "Lanluma behandeling voor een gladde huid",
                "content" => "
                    <p>De Lanluma behandeling is ideaal voor het verkrijgen van een gladde huid of als aanvulling op een brazilian butt lift. Een bijkomend voordeel van deze behandeling is dat het de aanmaak van collageen in het lichaam stimuleert, wat resulteert in een gladdere huid. Dit maakt de behandeling bij The Golden Glow in Amsterdam bijzonder geschikt voor mensen met cellulitis. Hoewel de collageenproductie enige tijd nodig heeft om op gang te komen, wordt het optimale resultaat meestal na 3-6 maanden zichtbaar. Bovendien kan de Lanluma behandeling ook na een brazilian butt lift worden toegepast. Door gerichte injecties is het mogelijk om de nieuwe vorm van de billen verder te verfijnen, wat leidt tot een nog beter eindresultaat!</p>
                ",
                "show_background" => false,
                "custom_class" => "!py-0",
            ]
        ],
        [
            "type" => "price",
            "data" => [
                "title" => "Wat is de prijs van een Lanluma behandeling",
                "price" => "750",
                "anesthesia" => "met crème en/of injectie",
                "checkup" => "2 maanden",
                "effect_duration" => "ongeveer 2 jaar",
                "treatment_duration" => "30 minuten"
            ]
        ],
        [
            "type" => "happy-customers",
            "data" => [
                "title" => "Resultaat van een Lanluma behandeling",
                "content" => "De effecten van de Lanluma-behandeling zijn langdurig en kunnen tot wel 2 jaar aanhouden, waardoor je langdurig van de resultaten kunt genieten. Het geleidelijke effect van de behandeling betekent dat de collageenstimulatie voortdurend blijft werken in je huid, wat resulteert in een natuurlijk ogende, stevige en stralende huid. Gedurende deze periode hoef je niets extra's te doen, aangezien je huid van binnenuit rustig doorgaat met het proces. Om het beste resultaat te bereiken, raden we aan om gedurende de eerste 2 weken na de behandeling dagelijks 2 keer 10 minuten je huid te masseren. Hierdoor verspreid je de filler optimaal over het behandelde gebied, wat bijdraagt aan een nog mooiere uitstraling.",
                "images" => [
                    ["filename" => "biostimulatie-happy-customer-1.png", "alt" => "Lanluma voor en na foto met zichtbaar volumevergroting"]
                ],
                "custom_class" => "!pt-0"
            ]
        ],
        [
            "type" => "after-treatment",
            "data" => [
                "custom_class" => "!py-0",
            ]
        ],
        [
            "type" => "treatment-summary",
            "data" => [
                "price" => "Vanaf 750",
                "duration" => "30 minuten",
                "checkup" => "2 maanden",
                "effect" => "ongeveer 2 jaar"
            ]
        ],
        [
            "type" => "appointment-button",
            "data" => [
                "url" => "https://schedule.clinicminds.com/services?clinic=a797764d-6a99-11ed-9e8e-0a42d89bf169&l=nl-NL",
                "label" => "Maak een afspraak"
            ]
        ],

        [
            "type" => "reviews",
            "data" => [
                "show_background" => true
            ]
        ],
        [
            "type" => "clinic",
            "data" => []
        ],
        [
            "type" => "faq",
            "data" => [
                "title" => "Veelgestelde vragen over Lanluma",
                "items" => [
                    [
                        "question" => "Welke zones kunnen met Lanluma behandeld worden?",
                        "answer" => "<p>Lanluma is vooral populair voor:<br>1) Billift zonder operatie<br>2) Heupen en dijen<br>3) Armen<br>4) Knieën<br>5) Cellulite-gebieden<br>6) Littekens of contourverbetering na gewichtsverlies<br><br>De behandeling is veelzijdig en wordt afgestemd op jouw lichaam en doelen.</p>"
                    ],
                    [
                        "question" => "Hoeveel Lanluma-behandelingen heb ik nodig?",
                        "answer" => "<p>Gemiddeld zijn 1 tot 2 sessies nodig, afhankelijk van het gewenste resultaat en de te behandelen zone. Bij grotere lichaamsgebieden, zoals de billen, zijn meestal 2 sessies nodig. Tijdens het consult wordt een persoonlijk behandelplan opgesteld.</p>"
                    ],
                    [
                        "question" => "Wanneer zie ik resultaat van Lanluma?",
                        "answer" => "<p>De eerste subtiele resultaten zijn al zichtbaar na enkele weken. Het volledige effect ontwikkelt zich geleidelijk over 2 tot 3 maanden, naarmate je lichaam meer collageen aanmaakt. Het resultaat ziet er natuurlijk uit en verbetert je contouren op een subtiele manier.</p>"
                    ],
                    [
                        "question" => "Hoe lang blijft het resultaat van Lanluma zichtbaar?",
                        "answer" => "<p>De resultaten van Lanluma kunnen tot 2 jaar aanhouden. Omdat het lichaam op natuurlijke wijze collageen blijft aanmaken, ziet de huid er langdurig stevig en volumineus uit. Onderhoudsbehandelingen zijn mogelijk.</p>"
                    ]
                ]
            ]
        ]
    ]
];
