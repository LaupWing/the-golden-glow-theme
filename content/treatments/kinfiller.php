<?php

/**
 * Content for: Kinfiller
 * To edit this page, just change the values in the content_blocks array below!
 * Each block has a "type" that determines which template to use.
 */

return [
    "page_title" => "Kinfiller behandeling",

    // Content blocks array - add, remove, or reorder as needed
    "content_blocks" => [
        [
            "type" => "category-intro",
            "data" => [
                "title" => "De kinfiller van Lisa",
                "content" => "
                    <p>Toen Lisa bij ons kwam, vertelde ze dat ze zich stoorde aan de contour van haar kin. Ze vond dat haar profiel niet helemaal in balans was en dat haar kin te kort leek in verhouding tot de rest van haar gezicht. Hierdoor kreeg ze soms een wat \"ronde\" uitstraling, terwijl ze zich eigenlijk juist krachtig en vrouwelijk voelt. Onze arts heeft samen met Lias gekeken naar de beste optie om de kin subtiel te verbeteren. Met een zachte hyaluronzuurfiller hebben we de kin iets verlengd en meer definitie aangebracht, waardoor er een mooiere balans ontstond tussen de kaaklijn, lippen en neus. Het resultaat is dat Lisa's gezichtsharmonie sterker naar voren komt en haar uitstraling beter aansluit bij hoe zij zichzelf ziet. Ze ervaart dat haar profiel strakker oogt en voelt zich hierdoor zelfverzekerder. Lisa is heel tevreden en komt terug wanneer ze merkt dat het effect langzaam afneemt.</p>
                ",
                "image_filename" => "lisa.jpg"
            ]
        ],
        [
            "type" => "text-content",
            "data" => [
                "title" => "De techniek achter het behandelen met een kinfiller",
                "content" => "
                    <p>Bij The Golden Glow in Amsterdam behandelen wij de kin vaak met hyaluronzuurfillers, zoals Belotero Volume of Restylane Lyft, omdat deze fillers stevigheid bieden en mooi integreren met het omliggende weefsel. Vooraf analyseert de arts zorgvuldig de gezichtslijnen en het profiel. Afhankelijk van de behoefte kan de filler worden geplaatst op de punt van de kin voor verlenging, aan de zijkanten voor meer breedte of juist centraal voor extra projectie. De behandeling start met een klein prikgaatje, waarna een fijne canule of dunne naald wordt gebruikt om de filler veilig en gecontroleerd te verdelen. Deze techniek maakt de behandeling comfortabel en verkleint de kans op blauwe plekken en zwelling. Door te werken met kleine hoeveelheden ontstaat een evenwichtig resultaat: de kin krijgt meer vorm en definitie zonder overdreven of onnatuurlijk effect.</p>
                ",
                "show_background" => true
            ]
        ],
        [
            "type" => "navigation",
            "data" => [
                "links" => [
                    ["href" => "#behandeling", "label" => "Behandeling"],
                    ["href" => "#prijzen", "label" => "Prijzen"],
                    ["href" => "#resultaat", "label" => "Resultaat"],
                    ["href" => "#nazorg", "label" => "Nazorg"]
                ]
            ]
        ],
        [
            "type" => "text-content",
            "data" => [
                "title" => "Wat u kunt verwachten na een kinfillerbehandeling",
                "content" => "
                    <p>Een kinfillerbehandeling duurt meestal 20 tot 30 minuten en is vrijwel pijnloos. Direct na afloop kan het behandelde gebied wat rood, gevoelig of licht gezwollen zijn. Dit trekt meestal binnen enkele dagen vanzelf weg. Voor een voorspoedig herstel adviseren wij om de eerste 24 uur te letten op rust voor de huid: geen make-up, geen intensief sporten en geen bezoek aan sauna of zonnebank. Ook alcohol en bloedverdunnende medicatie kunnen beter tijdelijk vermeden worden om blauwe plekjes te voorkomen. Het resultaat van een kinfiller is direct zichtbaar: de kin krijgt meer projectie en het profiel oogt harmonieuzer. Afhankelijk van de stofwisseling blijft het effect gemiddeld 12 tot 18 maanden behouden. Na verloop van tijd kan een herhaalbehandeling gewenst zijn om het resultaat te onderhouden.</p>
                ",
                "show_background" => false
            ]
        ],
        [
            "type" => "price",
            "data" => [
                "title" => "Wat is de prijs van een kinfiller behandeling",
                "price" => "275",
                "anesthesia" => "met crème en/of injectie",
                "checkup" => "na 2 weken",
                "effect_duration" => "6 - 12 maanden",
                "treatment_duration" => "30 minuten"
            ]
        ],
        [
            "type" => "after-treatment",
            "data" => []
        ],
        [
            "type" => "treatment-summary",
            "data" => [
                "price" => "Vanaf 275",
                "duration" => "30 minuten",
                "checkup" => "na 2 weken",
                "effect" => "6 - 12 maanden"
            ]
        ],
        [
            "type" => "appointment-button",
            "data" => [
                "url" => "https://schedule.clinicminds.com/services?clinic=a797764d-6a99-11ed-9e8e-0a42d89bf169&l=nl-NL",
                "label" => "Maak een afspraak"
            ]
        ],
        [
            "type" => "reviews",
            "data" => [
                "show_background" => true
            ]
        ],
        [
            "type" => "clinic",
            "data" => []
        ],
        [
            "type" => "faq",
            "data" => [
                "title" => "Veelgestelde vragen over fillerbehandelingen",
                "items" => [
                    [
                        "question" => "Wat zijn kinfillers?",
                        "answer" => "<p>Kinfillers zijn injecties met hyaluronzuur die worden gebruikt om de vorm, lengte of projectie van de kin te verbeteren. Ze zorgen voor een betere balans in het gezicht en kunnen bijdragen aan een strakkere kaaklijn en harmonieuzere gezichtscontouren.</p>"
                    ],
                    [
                        "question" => "Wat doen kinfillers precies?",
                        "answer" => "
                            <ul>
                                <li><p>Verbeteren van de vorm: corrigeert een terugvallende of kleine kin.</p></li>
                                <li><p>Betere gezichtsbalans: brengt de kin in verhouding met neus, lippen en kaaklijn.</p></li>
                                <li><p>Strakkere kaaklijn: geeft extra definitie en een krachtigere uitstraling.</p></li>
                            </ul>
                        "
                    ],
                    [
                        "question" => "Hoe lang blijft het resultaat zichtbaar?",
                        "answer" => "<p>Kinfillers blijven gemiddeld 6 tot 12 maanden mooi zichtbaar. Dit verschilt per persoon en hangt af van huidtype, stofwisseling en leefstijl.</p>"
                    ]
                ]
            ]
        ]
    ]
];
