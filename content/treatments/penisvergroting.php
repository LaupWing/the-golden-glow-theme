<?php

/**
 * Content for: Penisvergroting
 * To edit this page, just change the values in the content_blocks array below!
 * Each block has a "type" that determines which template to use.
 */

return [
    "page_title" => "Penisvergroting",

    // Content blocks array - add, remove, or reorder as needed
    "content_blocks" => [
        [
            "type" => "category-intro",
            "data" => [
                "title" => "Penisvergroting met filler & echogeleide techniek",
                "content" => "
                    <p>Bij The Golden Glow bieden wij een penisvergroting met filler aan als een veilige, niet-chirurgische behandeling die gericht is op een toename van volume, met behoud van natuurlijke proporties en functionaliteit. Dankzij het gebruik van echogeleid injecteren kunnen wij de filler met grote precisie en controle aanbrengen — een techniek die zorgt voor maximale veiligheid en een optimaal resultaat.</p>
                    <p>Door het injecteren van filler-gel onder lokale verdoving kan de schacht van de penis, buiten de zwellichamen, subcutaan worden verbreed. Dit resulteert in een zachte, natuurlijke aanvoelende vergroting die  zichtbaar is in erectie als in niet erectie toestand.</p>
                ",
                "image_filename" => "penisvergroting.jpg"
            ]
        ],
        [
            "type" => "text-content",
            "data" => [
                "title" => "Vergroting van de eikel – esthetisch in balans",
                "content" => "
                    <p>Ook het vergroten van de eikel (glans) behoort tot de mogelijkheden. Met behulp van goede anesthesie en fijne injectietechniek wordt de omtrek vergroot, zodat deze in verhouding blijft tot de rest van de penis. De behandeling is vrijwel pijnloos en kan desgewenst gecombineerd worden met de schachtverdikking.</p>
                    <p>Hoewel echogeleiding bij glansverdikking niet standaard noodzakelijk is, passen wij deze waar mogelijk ook hier toe, om maximale controle en symmetrie te garanderen.</p>
                ",
                "show_background" => true
            ]
        ],
        [
            "type" => "navigation",
            "data" => [
                "links" => [
                    ["href" => "#behandeling", "label" => "Behandeling"],
                    ["href" => "#prijzen", "label" => "Prijzen"],
                    ["href" => "#resultaat", "label" => "Resultaat"],
                    ["href" => "#nazorg", "label" => "Nazorg"]
                ]
            ]
        ],
        [
            "type" => "text-content",
            "data" => [
                "title" => "Hoe vindt de behandeling plaats van een penisverdikking?",
                "content" => "
                    <p>De filler wordt onder lokale verdoving geïnjecteerd in de schacht van de penis, buiten de zwellichamen. De filler wordt in een specifieke laag strategisch geplaatst en middels een echo wordt de filler op specifieke anatomische plekken ingebracht voor een optimaal resultaat. De erectiele functie wordt niet beïnvloed, terwijl in zowel ontspannen als erectie toestand een bredere, vollere contour ontstaat. Doordat wij standaard echogeleiding toepassen, kunnen we exact zien waar de filler zich bevindt, en indien nodig de plaatsing bijsturen tijdens de behandeling zelf. Dit voorkomt onregelmatigheden en minimaliseert risico's.</p>
                ",
                "show_background" => true
            ]
        ],
        [
            "type" => "text-content",
            "data" => [
                "title" => "Discretie, expertise en medische precisie dankzij echogeleid injecteren",
                "content" => "
                    <p>Onze aanpak is doordacht, precies en afgestemd op de cliënt. Door gebruik te maken van echogeleid injecteren — een standaard binnen onze kliniek — waarborgen we een hoge mate van veiligheid, nauwkeurigheid en voorspelbaarheid. Dokter Arwind Chigharoe heeft ruime ervaring in intieme esthetiek en werkt met oog voor detail en respect voor de anatomie.</p>
                    <p>De behandeling vindt plaats in een discrete en professioneel ingerichte omgeving in hartje Amsterdam. In een persoonlijk consult nemen wij ruim de tijd om uw wensen en verwachtingen te bespreken, en ontvangt u een eerlijk en deskundig advies.</p>
                    <p>Bij The Golden Glow streven wij naar verfijnde, natuurlijk ogende resultaten, waarbij veiligheid en precisie vooropstaan. Echogeleide injectietechniek is voor ons geen optie, maar een standaard — zodat u verzekerd bent van zorg op het hoogste niveau.</p>
                ",
                "show_background" => false
            ]
        ],
        [
            "type" => "text-content",
            "data" => [
                "title" => "Penisvergroting met een collageen booster",
                "content" => "
                    <p>Penisvergroting met een collageen booster biedt een niet-chirurgische oplossing voor mannen die de omtrek van hun penis willen vergroten. De collageen booster, een biocompatibele en biologisch afbreekbare filler, wordt subcutaan geïnjecteerd om geleidelijk en natuurlijk stevigheid toe te voegen. Dit resulteert tevens ook in een verbeterde tevredenheid tijdens seksuele activiteit. De procedure wordt uitgevoerd onder lokale verdoving en heeft minimale hersteltijd. Ontdek hoe penisvergroting met een collageen booster uw zelfbeeld en intieme leven kan verbeteren.</p>
                ",
                "show_background" => true
            ]
        ],
        [
            "type" => "text-content",
            "data" => [
                "title" => "Penisvergroting met hyaluronzuur",
                "content" => "
                    <p>Penisvergroting met hyaluronzuur is een niet-invasieve optie voor mannen die streven naar een vollere penisomtrek en een toename van de penislengte. Hyaluronzuurfiller wordt subcutaan geïnjecteerd om geleidelijk volume toe te voegen en een natuurlijke uitstraling te behouden. Tevens wordt het hyaluronzuur strategisch bij bepaalde ligamenten inspoten voor optimaal moduleren van de penis. Deze behandeling resulteert in verbeterd comfort en zelfvertrouwen tijdens intieme momenten. Uitgevoerd onder lokale verdoving, biedt de procedure minimale hersteltijd en een langdurig resultaat. Ontdek hoe penisvergroting met hyaluronzuur uw seksuele ervaring kan versterken en uw zelfvertrouwen kan vergroten.</p>
                ",
                "show_background" => true
            ]
        ],
        [
            "type" => "text-content",
            "data" => [
                "title" => "Combinatie van hyaluronzuur en collageen booster voor penisvergroting",
                "content" => "
                    <p>Zowel hyaluronzuur als de collageen booster bieden effectieve oplossingen voor penisvergroting, elk met unieke voordelen die bijdragen aan een optimaal resultaat. Een hyaluronzuurfiller staat bekend om zijn vermogen om direct volume toe te voegen, waardoor direct zichtbare resultaten ontstaan. Aan de andere kant stimuleert de collageen booster de natuurlijke collageenproductie van het lichaam, wat resulteert in een geleidelijke, maar langdurige toename van de omvang. Door deze complementaire benaderingen te combineren, kunnen patiënten profiteren van zowel directe als langdurige verbeteringen, wat resulteert in een algeheel verbeterd esthetisch resultaat. Met de deskundige begeleiding van een ervaren cosmetisch arts kunnen hyaluronzuur en collageen booster samen worden gebruikt om een optimaal en duurzaam resultaat te bereiken. Hierdoor kunnen patiënten genieten van een bevredigende esthetische en seksuele ervaring.</p>
                ",
                "show_background" => false
            ]
        ],
        [
            "type" => "price",
            "data" => [
                "title" => "Penisvergroting",
                "price" => "1200/2500",
                "anesthesia" => "met crème en/of injectie",
                "checkup" => "na 2 weken",
                "effect_duration" => "1 - 1.5 jaar",
                "treatment_duration" => "60 minuten"
            ]
        ],
        [
            "type" => "text-content",
            "data" => [
                "title" => "Resultaat van een penis vergroting behandeling",
                "content" => "
                    <p>Het resultaat is afhankelijk van de hoeveelheid toepaste filler. Bij de meeste cliënten is een behandeling met 10 ml hyaluronzuur voldoende. Door toevoeging van de collageenbooster blijft het effect van de hyaluronzuurfiller ca. zes maanden langer behouden. Bent u geïnteresseerd naar wat een penisvergroting voor u kan betekenen, dan zien wij u graag voor een consult.</p>
                ",
                "show_background" => false
            ]
        ],
        [
            "type" => "happy-customers",
            "data" => [
                "title" => "Onze tevreden klanten",
                "content" => "",
                "images" => [
                    ["filename" => "body-filler-happy-customer-1.png", "alt" => "Penisvergroting resultaat"],
                    ["filename" => "penisvergroting-happy-customer-1.jpg", "alt" => "Penisvergroting resultaat"],
                    ["filename" => "penisvergroting-happy-customer-2.jpg", "alt" => "Penisvergroting resultaat"]
                ]
            ]
        ],
        [
            "type" => "text-content",
            "data" => [
                "title" => "BELANGRIJKE DETAILS VOOR NA DE BEHANDELING",
                "content" => "
                    <p>Na de behandeling kunt u uw lichte dagelijkse activiteiten gewoon hervatten. U krijgt duidelijke instructies mee voor de verzorging van de wond. In de eerste 24 uur mag u niet douchen, geen alcohol of koffie drinken en het behandelde gebied niet aanraken. Gedurende de eerste twee weken wordt afgeraden om te sporten, zware inspanning te leveren of seksuele activiteiten te hebben. De eerste twee weken na de behandeling dient u sauna, stoombad, zonnebank, hete douches te vermijden. Door deze adviezen op te volgen, bevordert u een goed herstel. Neem bij vragen gerust contact op met uw arts.</p>
                ",
                "show_background" => false
            ]
        ],
        [
            "type" => "treatment-summary",
            "data" => [
                "title" => "Hyaluronzuur",
                "price" => "€2500",
                "duration" => "60 minuten",
                "checkup" => "2 weken",
                "effect" => "12 maanden"
            ]
        ],
        [
            "type" => "treatment-summary",
            "data" => [
                "title" => "CollageenBooster",
                "price" => "€1200",
                "duration" => "60 minuten",
                "checkup" => "2 weken",
                "effect" => "12 maanden"
            ]
        ],
        [
            "type" => "appointment-button",
            "data" => [
                "url" => "https://schedule.clinicminds.com/services?clinic=a797764d-6a99-11ed-9e8e-0a42d89bf169&l=nl-NL",
                "label" => "Maak een afspraak"
            ]
        ],
        [
            "type" => "reviews",
            "data" => [
                "show_background" => true
            ]
        ],
        [
            "type" => "clinic",
            "data" => []
        ],
        [
            "type" => "faq",
            "data" => [
                "title" => "Veelgestelde vragen over penisvergroting",
                "items" => [
                    [
                        "question" => "Zijn er hechtingen nodig",
                        "answer" => "<p>Er zijn geen hechtingen nodig. Er worden kleine gaatjes gemaakt door de naald om de vuloplossing in te spuiten.</p>"
                    ],
                    [
                        "question" => "Hoe lang duurt het voor dat ik de resultaten zie",
                        "answer" => "<p>Uw penis zal na de behandeling onmiddellijk dikker zijn, dankzij het vulmiddel dat aan de penis is toegevoegd.</p>"
                    ],
                    [
                        "question" => "Wanneer mag ik een herhaalbehandeling ondergaan?",
                        "answer" => "<p>Meestal blijft het effect van de behandeling tot wel 2 jaar zichtbaar. Mocht u een aanvullende of een herhaalbehandeling wensen, volg dan het advies op van de arts.</p>"
                    ]
                ]
            ]
        ]
    ]
];
